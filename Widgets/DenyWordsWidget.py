...
class DenyWordsWidget(QWidget):
    def __init__(self):
        super(DenyWordsWidget, self).__init__()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        hint_label = QLabel("如输入”別スキャン“，则所有名称中含有”別スキャン“的文件夹均会被移除。\n如果此处不输入任何内容，则默认保留所有文件夹。")
        ...
        for word in global_settings.deny_words:
            self.show_list.addItem(word)
        layout_main.addWidget(self.show_list)
        self.setLayout(layout_main)

    def add_folder_button_clicked(self):
        new_path = QInputDialog.getText(self, "否决词设置", "请输入新的一票否决词", QLineEdit.Normal)
        if new_path[0] and new_path[1]:
            for word in global_settings.deny_words:
                if word == new_path[0]:
                    return
            global_settings.deny_words.append(new_path[0])
    ...
    def delete_folder_button_clicked(self):
        if self.show_list.currentItem() is None:
            return
        global_settings.deny_words.remove(self.show_list.currentItem().text())
        self.show_list.clear()
        for word in global_settings.deny_words:
            self.show_list.addItem(word)
